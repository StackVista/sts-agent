    ...
    def handleUdpSpan(self, header, body, addr):
        try:
            ...
            if(type == 40):
                ...
                fromName = fromIdentifier
                log.info("from: " +fromIdentifier)
                self.component(self.instanckey, fromIdentifier, {"name": "service"}, {"tags": [], "instance_id": fromIdentifier  })
                if(tSpan.spanEventList != None and tSpan.spanEventList[0].destinationId != None):
                    toIdentifier = tSpan.spanEventList[0].destinationId.replace(":", "_")
                    log.info("to: " + toIdentifier)
                    self.component(self.instanckey, toIdentifier, {"name": "service"}, {"tags": [], "instance_id": toIdentifier})
                    self.relation(self.instanckey, fromIdentifier, toIdentifier, {"name": "servicecall"}, data={ "from": fromIdentifier, "to": toIdentifier})
                    tags = []
